...
public class DynamicArrayWithAddMethod {
    ...
    public DynamicArrayWithAddMethod() {
        array = new int[1];
        count = 0;
        size = 1;
    }
    ...
    // function to add an array of elements at the end of the dynamic array
    public void addArray(int[] arrAdd) {
        int newSize = count + arrAdd.length;
        if (newSize > size) {
            // If the new size exceeds the current size, resize the array
            while (newSize > size) {
                growSize();
            }
        }
        // Add elements of the array at the end of the dynamic array
        for (int i = 0; i < arrAdd.length; i++) {
            array[count++] = arrAdd[i];
        }
    }
